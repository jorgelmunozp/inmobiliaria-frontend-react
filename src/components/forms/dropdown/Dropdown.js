export const Dropdown = ({ value,query,parameters,setQuery,className }) => {
    return(
      <div className="dropdown form-floating w-100">
        <button className={className + (query === '' ? ' dropdown-toggle text-start pt-2 ps-2 ps-sm-3 pe-5 w-100' : ' dropdown-toggle text-center pt-4 ps-2 ps-sm-3 pe-5 w-100')} type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
          { query === '' ? value : query }
        </button>
